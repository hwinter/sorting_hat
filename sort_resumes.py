...
#set up so it reads directly from download (i.e. ignoring none standard characters
    ...
    def dict_adv(self): #create a dictionary of advisors for a given project
        for i in self.projects: #loop overall projects
            ...
            advisor = proinfo[-1].replace(')','').replace('Dr. Henry Winter III and Dr. Steve Saar','Dr. Adam Foster and Dr. Henry Winter III').replace('Ed Hertz','Dr. Henry Winter III').replace('Peter Cheimets','Jenna Samra') #make advisor list for given project. staticed in replacements 
